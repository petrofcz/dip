...
\clearpage
	...
	\section{Funkční požadavky}
		...
		U nástrojů a efektů bude možné vytvořit snímek všech parametrů pluginu, který bude považován za výchozí a bude načten při spuštení aplikace, nebo po přepnutí zvuku. V rámci seznamů bude též možné otevřít uživatelské rozhraní VST pluginu, či zobrazit dostupnost v jednotlivých vrstvách. Pro nástroje bude možnost definovat více pojmenovaných stránek mapování, pro efekt bude možné zadat právě jednu stránku mapování s volbou hlavního mapování.
		...
		Při volbě vztažného parametru pluginu bude možné použít tzv. funkce learn, kdy lze ovládaný parametr vybrat přímo v prostředí pluginu.
		Bude možné zadat název mapování.
		Pro každé mapování budou k dispozici následující strategie:
		\begin{itemize}
			\item\label{LinearMappingStrategy} \textbf{Lineární} - Tato strategie bude umožňovat určit lineární vztah mezi hodnotou enkoderu na hardwarovém kontroleru 
			a hodnotou parametru příslušného pluginu. Bude možné zadat rozsah zobrazovaných hodnot, rozsah hodnot parametru VST pluginu, invertovat
			...
			umožňuje hodnoty zvolit přímo v prostředí VST pluginu. Na otočných enkodérech kontroleru bude stav lineárně indikován pro celý rozsah enkodéru.
			...
			\item \textbf{Seznam} - Za použití této strategie bude možné definovat seznam položek obsahující název a hodnotu parametru VST pluginu.
			Oproti lineární strategii s položkovým zobrazením bude probíhat změna parametru VST pluginu skokově. Při reflektování změn parametru bude zobrazena
			položka, která je hodnotou aktuální hodnotě parametru nejblíže.
		\end{itemize}
		...
		strategii inicializace pluginu vč. konfigurace, rozdílovou sadu hodnot parametrů pluginu a rozdílovou sadu hodnot parametrů efektů vrstvy.
		Rozdílová sada bude pro danou vrstvu vytvářena při změně v parametrech pluginu v době, kdy je plugin aktivní. Zaznamenanou sadu bude
		možné pro aktivní preset uložit.
		V rámci inicializace VST pluginu budou k dispozici následující strategie:
		\begin{itemize}
			\item \textbf{Snímek všech parametrů} - V této strategii bude možné pořídit snímek hodnot všech parametrů VST pluginu, který bude načten
			při načtení presetu.
		...
		\end{itemize}
		Pro každou vrstvu bude možný být aktivní právě jeden preset. Po jeho načtení bude aplikovaná uložená inicizalizační strategie.
		...
		historie posledních použitých presetů vč. svých stavů pro každou vrstvu.
		Hodnoty parametrů pluginů bude možné ovládat otočnými enkodéry kontroleru. Hodnoty budou indikovány LED prstencem enkodéru a grafickým výstupem na displayi.
		...
		Kontroler umožní přepínat právě ovládanou vrstvu. Za pomocí posuvných prvků kontroleru bude možné ovládat hlasitosti jednotlivých vrstev.
