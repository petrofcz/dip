...
\clearpage
	...
	\section{Struktura projektu Bidule}
...
\chapter{Implementace vlastního řešení}
	\section{Komunikace přes USB}
		...
		Třída \texttt{MidiAdapter} tak poskytuje částečnou
		implementaci tohoto protokolu. Třída \texttt{USBDriver} poskytuje jednoduché rozhraní pro komunikaci 
		s hardwarovým kontrolerem připojeným přes USB.

	\section{Stavový model aplikace a knihovna NGXS}
		...
		Model stavu aplikace je zobrazen na diagramu TODO. Struktura stavu je normalizovaná z důvodu zamezení duplicity dat
		a zajištění datové konzistence\cite{state-normalization}.
